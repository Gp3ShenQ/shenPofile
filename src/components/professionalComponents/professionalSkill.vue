<template>
  <p class="border-[#d1d1e0] pt-20 pb-5 border-b-[3px] w-full max-w-[600px] h-full text-3xl text-black text-left page-title-font">Skill</p>
        ...
        <div class="w-full h-full" :data-depth="positionAll(index)">
          <div class="grid grid-cols-2 grid-rows-10 w-full h-full" data-depth="0.1">
            <div :class="{ 'col-start-2 row-start-4': isEven(index), 'row-start-8': isLastData(index), 'row-start-6': notFirstOdd(index) }">
              <div class="relative flex flex-col justify-center items-center bg-white shadow-[0_10px_20px_#00000050] p-5 max-l:p-1 w-full max-w-[300px] min-h-[175px]">
